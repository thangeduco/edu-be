...
exports.PgCourseWeekDetailQuery = void 0;
...
class PgCourseWeekDetailQuery {
    ...
    async getCourseWeekDetailForPublicView(input) {
        ...
        try {
            ...
            if (!header) {
                throw new Error(`[PgCourseWeekDetailQuery] Không tìm thấy khoá học với mã = ${courseCode}`);
            }
            ...
            console.error('[PgCourseWeekDetailQuery][getCourseWeekDetailForPublicView] ❌ Lỗi khi truy vấn chi tiết khoá học', err);
            throw err;
        }
    }
    /**
     ...
     */
    async fetchCourseHeader(courseCode) {
        try {
            const sql = `
        SELECT
          c.id,
        ...
        WHERE c.code = $1
      `;
            const result = await pgClient_1.pgPool.query(sql, [courseCode]);
            ...
            const header = {
                ...
                totalVideoLessonsCount: Number(row.total_video_lessons_count ?? 0),
                totalWorksheetLessonsCount: Number(row.total_worksheet_lessons_count ?? 0),
            };
            return header;
        }
        catch (err) {
            console.error('[PgCourseWeekDetailQuery][fetchCourseHeader] ❌ Lỗi khi truy vấn header khoá học', err);
            throw err;
        }
    }
    /**
     ...
     *  - body.weeks
     */
    async fetchSidebarAndBody(courseCode) {
        try {
            const sql = `
        SELECT
          w.id AS week_id,
      ...
      `;
            ...
            const sidebarWeeksMap = new Map();
            const bodyWeeksMap = new Map();
            // Map lessonId -> BodyLessonDTO (theo từng week)
            const bodyLessonsMapByWeek = new Map();
            // Map lessonId -> Set videoId / worksheetId để tránh duplicate
            const videoIdSetByLesson = new Map();
            const worksheetIdSetByLesson = new Map();
            for (const row of rows) {
                ...
                if (!bodyWeek) {
                    ...
                    bodyLessonsMapByWeek.set(weekId, new Map());
                }
                ...
                const isLessonAlreadyInSidebar = sidebarLessons.some((l) => l.id === lessonId);
                ...
                if (!videoIdSetByLesson.has(lessonId)) {
                    videoIdSetByLesson.set(lessonId, new Set());
                }
                if (!worksheetIdSetByLesson.has(lessonId)) {
                    worksheetIdSetByLesson.set(lessonId, new Set());
                }
                ...
                // ================== BODY: WORKSHEETS ==================
                const worksheetId = row.worksheet_id;
            ...
            console.error('[PgCourseWeekDetailQuery][fetchSidebarAndBody] ❌ Lỗi khi truy vấn weeks/lessons/video/worksheets', err);
            throw err;
        }
    }
}
exports.PgCourseWeekDetailQuery = PgCourseWeekDetailQuery;
