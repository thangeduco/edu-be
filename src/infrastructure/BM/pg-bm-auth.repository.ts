...
import { BMUser } from '../../domain/BM/models/user.model';
...
export class PgAuthRepository implements AuthRepository {
  ...
  async findUserByEmailOrPhone(identifier: string): Promise<BMUser | null> {
  ...
  async getUserById(userId: number): Promise<BMUser | null> {
    const res = await pgPool.query(
      `SELECT 
        u.*, 
        p.avatar_image, 
  ...
  async createUser(user: Partial<BMUser>): Promise<BMUser> {
    ...
    try {
      ...
      const userInsertResult = await client.query(
        `INSERT INTO bm_users (full_name, email, phone, password_hash, role, status, created_at)
      ...
      const grade =
        typeof profile.grade === 'number' && !Number.isNaN(profile.grade)
          ? profile.grade
          : null;
      ...
      await client.query(
        `INSERT INTO bm_user_profiles (user_id, avatar_image, grade, gender, dob, slogen)
         VALUES ($1, $2, $3, $4, $5, $6)`,
  ...
  async createSession(
    ...
    try {
      ...
      await client.query(
        `INSERT INTO bm_user_session_logs (
            user_id,
            session_id,
            token,
            device,
  ...
  async endSessionByToken(
    ...
    try {
      ...
      const res = await client.query(
        `SELECT id, user_id, device, ip_address, user_agent
      ...
      if (!session) {
        ...
        await client.query('ROLLBACK');
        console.warn(
          '[PgAuthRepository][endSessionByToken] Không tìm thấy session với token',
          token
        );
        return;
      }
      ...
      await client.query(
        `INSERT INTO bm_user_session_logs (
         ...
         VALUES ($1, $2, $3, $4, 'SESSION_END', $5, $6, NOW())`,
        [userId, sessionId, token, finalDevice, finalIp, finalUa]
      );
  ...
  private mapUserWithProfile(row: any): BMUser {
    const createdAt =
