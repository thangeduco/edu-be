...
import { IProductRepo, ISubscriptionRepo } from '../../../../domain/BM/repos';
import { BmProduct, BmSubscription } from '../../../../domain/BM/models/ProductModels';

import {
  SubscribePaidCourseInput,
  SubscribeTrialCourseInput,
  SubscribeCourseResult,
 ...
 */
export class SubscribeProductUC {
  constructor(
   ...
   */
  async subscribePaidCourse(
    input: SubscribePaidCourseInput
  ): Promise<SubscribeCourseResult> {
    const { studentId, productCode } = input;
    console.info(
      `[SubscribeProductUC][subscribePaidCourse] 👉 studentId=${studentId}, productCode=${productCode}`
    );
    ...
    const product: BmProduct = await this.productRepo.findByProductCode(productCode);
    ...
    const createdSub: BmSubscription =
    ...
    const result: SubscribeCourseResult = {
   ...
   */
  async subscribeTrialCourse(
    input: SubscribeTrialCourseInput
  ): Promise<SubscribeCourseResult> {
    const { studentId, productCode } = input;
    console.info(
      `[SubscribeProductUC][subscribeTrialCourse] 👉 studentId=${studentId}, productCode=${productCode}`
    );
    ...
    const trialDays =
      input.trialDaysOverride ??
      product.trialDays ??
      0;

    if (!trialDays || trialDays <= 0) {
      throw new Error(
        `Product code=${productCode} không cấu hình số ngày trial hợp lệ`
      );
    }
